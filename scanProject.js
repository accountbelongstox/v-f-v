...
const ignoredExtensions = ['.log',`.woff2`,`.txt`,`.md`,`.png`,`.jpg`,`.css`];  // 例如：跳过.log文件
const ignoredDirectories = ['node_modules',`build`,`.git`,`.vscode`,`.next`,`public`];  // 例如：跳过node_modules目录
...
function scanDirectory(dir, prefix = '', includeFiles = false) {
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        if (entry.isDirectory() && !ignoredDirectories.includes(entry.name) && !entry.name.startsWith('.')) {
        ...
        } else if (includeFiles && entry.isFile() && !ignoredExtensions.includes(path.extname(entry.name)) && !entry.name.startsWith('.')) {
            output += `${prefix}${isLast ? '└─' : '├─'} ${entry.name}\n`;
        }
    }
